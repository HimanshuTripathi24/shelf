...
export default function AuthPage() {
  ...
  async function handleEmailAuth(e: React.FormEvent) {
    ...
    if (mode === "signup") {
      ...
      else { setMessage({ text: "Account created! You can now sign in.", type: "success" }); setMode("signin"); }
    } else {
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .google-btn { width: 100%; display: flex; align-items: center; justify-content: center; gap: 12px; background: #fff; color: #1a1a1a; border: none; border-radius: 8px; padding: 13px 20px; font-size: 14px; font-family: 'Georgia', serif; font-weight: 700; cursor: pointer; transition: all 0.2s; margin-bottom: 12px; }
        ...
        .guest-btn { width: 100%; display: flex; align-items: center; justify-content: center; gap: 10px; background: transparent; color: rgba(255,255,255,0.5); border: 1px solid rgba(255,255,255,0.12); border-radius: 8px; padding: 12px 20px; font-size: 13px; font-family: 'Georgia', serif; cursor: pointer; transition: all 0.2s; margin-bottom: 24px; letter-spacing: 0.06em; }
        .guest-btn:hover { border-color: rgba(255,255,255,0.25); color: rgba(255,255,255,0.75); transform: translateY(-1px); }
        .guest-btn:disabled { opacity: 0.5; cursor: not-allowed; transform: none; }
        .guest-note { font-size: 11px; color: rgba(255,255,255,0.2); text-align: center; margin-top: -18px; margin-bottom: 24px; letter-spacing: 0.04em; }
      ...
      <div className="auth-page">
        <div className="auth-box">
          ...
          <p className="auth-subtitle">{mode === "signin" ? "Sign in to sync your reading progress" : "Join to track and sync your novels"}</p>
          ...
          {/* Guest */}
          <button className="guest-btn" onClick={handleGuest} disabled={guestLoading}>
            👤 {guestLoading ? "Entering as guest..." : "Continue as Guest"}
          </button>
          <p className="guest-note">No account needed · Progress not saved after session ends</p>
